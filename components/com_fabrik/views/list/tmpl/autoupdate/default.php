...
?>
				...
				window.setInterval(function(){
					if ($('autorefresh').hasClass('active')) {
						if (VloRemote.timer !== false) {
							Fabrik.getBlock('list_<?php echo $this->table->id; ?>').updateRows();;
						}
					}
	...
	jQuery(document).ready(function() { 
		...
		requirejs(['fab/fabrik'], function() {
			Fabrik.addEvent('fabrik.list.update', function(list) {
				if (list.id == <?php echo $this->table->id; ?>) {
					var mydiv = "#fab_"+list.id+" .outro";
					jQuery(mydiv).css({
	...
	<?php print_r($this->hiddenFields);?>
        <div class="outro"><?php echo $this->table->outro/1000;echo "s";?></div>
</div>
</form>
<?php
if ($pageClass !== '') :
