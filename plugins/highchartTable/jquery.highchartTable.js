...
(function($) {
  $.fn.highchartTable = function() {
    ...
    var allowedGraphTypes = ['column', 'line', 'area', 'spline', 'pie', 'scatter'];
    ...
    this.each(function() {
      ...
      ths.each(function(indexTh, th) {
        ...
        var markersEnabled = $th.data('graph-markers-enabled');
        if (typeof markersEnabled == 'undefined') {
      ...
      var markerRadius   = $table.data('graph-marker-radius') || 2;
      var series = [];
      ...
      rows.each(function(indexRow, row) {
        if (!!$(row).data('graph-skip') || (typeof $(row).attr('id') == 'undefined')) {
          return;
        }
        var tds = $('td', row);
...
//                  name:   typeof tdGraphName != 'undefined' ? tdGraphName : rawCellValue,
                  name:   typeof tdGraphName != 'undefined' ? tdGraphName : cellValue,
                  y:      cellValue,
      ...
      var highChartConfig = {
        ...
        tooltip: {
            formatter: function() {
              if ($table.data('graph-xaxis-type') == 'datetime') {
                return '<b>'+ this.series.name +'</b><br/>'+  Highcharts.dateFormat('%b-%e %H:%M:%S', this.x) +' : '+ this.y;
              } else {
        ...
        plotOptions: {
          ...
          area: {
            ...
            fillOpacity: $table.data('graph-area-fillOpacity') || 0.25
          },
          pie: {
  ...
  var parseDate = function(datetime) {
    ...
    if(calculatedateInfos[1]) {
      ...
      hour               =  parseInt(dateHourInfos[0], 10);
      min              = parseInt(dateHourInfos[1], 10);
      sec               =  parseInt(dateHourInfos[2], 10);
    }
    if (isNaN(sec)) {
		return new Date(parseInt(dateDayInfos[0], 10), parseInt(dateDayInfos[1], 10)-1, parseInt(dateDayInfos[2], 10), hour, min);
	} else {
		return new Date(parseInt(dateDayInfos[0], 10), parseInt(dateDayInfos[1], 10)-1, parseInt(dateDayInfos[2], 10), hour, min, sec);
	}
  };
