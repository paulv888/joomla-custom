...
var timerTime = 200;
var pre_direction = 0;
var glb_identifier = 0;
var playing = 0;
var prev_time = 0;

function vsig_dom(obj) {return document.getElementById(obj); }

function  stop_button(direction, identifier) {
	var t_ident_b = window[identifier+'_b'];
	var topimg = "topimg" + t_ident_b[2];
	playing = 0;
	vsig_dom(topimg).onload = null;

}

function play_button(direction, speed, identifier) {
	playing = 1;
	if (direction == 0) {
		if (pre_direction == 0 ) {
			direction = 1;
	...
	glb_identifier = identifier;

	var t_ident_b = window[identifier+'_b'];
	var topimg = "topimg" + t_ident_b[2];
	vsig_dom(topimg).onload = change_frame_onload;
	change_frame_onload();
}

function change_frame_onload() {
var t_ident_b = window[glb_identifier+'_b'];
var topimg = "topimg" + t_ident_b[2];
...
//	}, timerTime);
	...
	if (pre_direction>0) {
		thumbidx = thumbidx+1;
		if(typeof window[glb_identifier][thumbidx] === 'undefined') {
			thumbidx=0;
		}
	} else {
		thumbidx = thumbidx-1;
...
console.log('change_onload');
//	if (playing) { 	// Do we need to slowdown since images already loaded

	switchimg(glb_identifier,thumbidx);
}

function  change_img_button(direction, identifier) {
	var t_ident_b = window[identifier+'_b'];
	var topimg = "topimg" + t_ident_b[2];
	var thumbidx = parseInt(vsig_dom(topimg).getAttribute("data-thumbid"));
