					...
					for($a = 0;$a<$noimage;$a++) {
						if($images[$a]['filename'] != '') {
							...
							if($images[$a]['filename']==$images[$aktimg]['filename']) {
								...
								$html2 .= "\n<img id='topimg".$identifier."' data-thumbid='' src='".$cur_webpath."' title='".$cur_alt."' alt='".$cur_alt."'/>";
								if($_link_use_&&isset($$vsiglinks)){ //links are activated and set
								$html2 .= "\n</a>";
								}
							...
							if(!$_sets_use_||($a>=($_sets_use_*$_sets_current_-$_sets_use_)&&$a<=($_sets_use_*$_sets_current_-1)&&$_sets_use_>=2)){
								...
								$html3 .= '<div id="_thbvsig_'.$identifier.'_'.$thumbs_in_set.'" class="vsig_cont vsig_cont'.$identifier.'"><div class="vsig_thumb">';
								$html3 .= '<a id="thbvsig_'.$identifier.'_'.$thumbs_in_set.'" href="'.$target[$identifier].'vsig'.$identifier.'='.$a.'" rel="nofollow"';
								if($_usescript_>=1){ //javascript is activated, insert the onclick event
								$html3 .= ' onclick=\'switchimg(vsig_'.$identifier.'['.$a.'],vsig_'.$identifier.'_b,'.$identifier.','.$a.');return false;\'';
									if($_hover_>=1){ //change main image on thumbhover too
									$html3 .= ' onmouseover=\'switchimg(vsig_'.$identifier.'['.$a.'],vsig_'.$identifier.'_b,'.$identifier.','.$a.');return false;\'';
									}
								}
								$html3 .= ' title="'.$cur_alt.'">';
...
//						$html4.="&nbsp;&nbsp;".$_sets_txt_."<span id='countervsig_".$identifier."' class='vsig_counter'>&nbsp;".$_sets_current_."/".$_sets_number_."</span>&nbsp;&nbsp;";
					...
					$html4.='</ul><ul class="pagination-list">';
					$html4.='<li class="prev-img"><a onclick="change_img(-1, vsig_'.$identifier.'_b); return false;" id="previmg_vsig_1_0" rel="nofollow" href="">Prev</a></li>';
					$html4.='<li class="next-img"><a onclick="change_img(1, vsig_'.$identifier.'_b); return false;" id="nextimg_vsig_1_0" rel="nofollow" href="">Next</a></li>';
					$html4.='</ul>';
